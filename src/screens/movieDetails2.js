...
import { TouchableOpacity, Text, View,FlatList, Image, Pressable, ImageBackground, ScrollView, SafeAreaView, StatusBar } from 'react-native'
...
import images1 from '../assets/play.png'
...
const MovieDetail1 = ({ navigation }) => {
    const movieId = navigation.state.params.movieId.id
    const [segmentIndex, setSegmentIndex] = useState(0)
    const [segmentIndex1, setSegmentIndex1] = useState(0)
    const [id, setId] = useState(0)
    const [videoUrl, setVideoUrl] = useState('')
    const [director, setDirectorName] = useState([])
    const [writers, setWirtersName] = useState([])
    const [cast, setCastName] = useState([])
    const [seasons, setSeasonsList] = useState([])
    const [eposideImage, setEposideListImage] = useState([])
    const [eposide, setEposideList] = useState([])
    const imageList = [
        {
    ...
    const handleIndexChange1 = index => {
        console.log(index)
        let eposideList=[]
        axios.get(`https://www.flicknplay.com/secure/titles/${movieId}`, { params: { titleId: movieId, titleName: navigation.state.params.movieId.name, seasonNumber: index + 1 } }).then((response) => {
            console.log(response.data.title.videos)
            eposideList=  response.data.title.videos.sort((a,b) => (a.episode_num > b.episode_num) ? 1 : ((b.episode_num > a.episode_num) ? -1 : 0))
            setEposideList(eposideList)
            setEposideListImage(response.data.title.season.episodes)
        })
        setSegmentIndex1(index)
    };
    useEffect(async () => {
        axios.get(`https://www.flicknplay.com/secure/titles/${movieId}`, { params: { titleId: movieId, titleName: navigation.state.params.movieId.name } }).then((response) => {
            
            let seasonsList = []
            let eposideList=[]
            if (response.data.title.seasons.length != 0) {
                axios.get(`https://www.flicknplay.com/secure/titles/${movieId}`, { params: { titleId: movieId, titleName: navigation.state.params.movieId.name, seasonNumber: 1 } }).then((response) => {
                    eposideList=  response.data.title.videos.sort((a,b) => (a.episode_num > b.episode_num) ? 1 : ((b.episode_num > a.episode_num) ? -1 : 0))
                    setEposideList(eposideList)
                    console.log(eposideList)
                   setEposideListImage(response.data.title.season.episodes)
                })
                response.data.title.seasons.map((data, index) => {
                    seasonsList.push(data.model_type + ' ' + data.number)
                })

            }
            if (response.data.title.videos.length != 0) {
                response.data.title.videos.map((data, item) => {
            ...
            let casts = []
            let writers = []
            if (response.data.title.credits.length != 0) {
                response.data.title.credits.map((data, item) => {

                    if (data.pivot.job === 'director') {

                        directors.push(data.name)
                    }
                    if (data.pivot.job === 'writer') {

                        writers.push(data.name)
                    }
                    if (data.pivot.job === 'cast') {

                        casts.push(data.name)
                    }
            ...
            <TouchableOpacity>
                <View>
                    <SegmentedControlTab
                        ...
                        values={seasons}
                        selectedIndex={segmentIndex1}
                        onTabPress={handleIndexChange1}
                    />
                </View>
            </TouchableOpacity>
        )
    ...
    const renderItem10 = ({ item }) => {
        ...
        return (
            <TouchableOpacity
            // onPress={()=>{console.log(eposide[item.episode_number-1])}}
                onPress={() => navigation.navigate('StackVideo', { movieDetail: eposide[item.episode_number-1].url })}
                >
      ...
      />
                ...
                <Text style={{textAlign:'center'}}>{'Eposide-'+item.episode_number}</Text>
            </TouchableOpacity>)
    }
    ...
    const renderItem = ({ item }) => {
        return (
            <View style={styles.item}>
                <Image source={{uri:item.imageName}} style={styles.images}></Image>
            </View>
    ...
    return (
        ...
        <ScrollView style={styles.scrollView}>
            <ImageBackground source={{ uri: data.title.backdrop }} resizeMode="cover" style={styles.images}>
                <BackButton style={{ zIndex: 1 }} goBack={() => navigation.navigate('TabNavigator')} />
                <View style={[styles.overlay]}>

                    <View style={styles.container}>
                        <Text style={{ paddingTop: 10, }}>{data.title.original_title}</Text>
                        <View >
                        ...
                        <Text >{data.title.genres.map(genre => {
                        ...
                        <View style={styles.play}>
                            ...
                            <Pressable style={styles.button} >
                                <Text style={styles.text} onPress={() => navigation.navigate('StackVideo', { movieDetail: videoUrl.url })}>{myIcon}Play</Text>
                            </Pressable>
                        ...
                        <Text style={styles.description}>{data.title.description}.</Text>
                        {
                            seasons.length != 0 ? (
                                <>
                                    <FlatList
                        ...
                        <View style={styles.tabbody}>
                            ...
                            {segmentIndex == 2 &&
                                <View style={styles.tabbody}>
                                    <View>
                                        <Text style={styles.marg}>Director</Text>
                                        <Text>{
                                            director.map((data, index) => (
                                                <span style={styles.letter}>
                                                    {data}
                                                </span>
                                            ))
                                        }</Text>
                                        ...
                                        {/* <hr style={styles.hrr} /> */}
                                        <Text style={styles.marg}>Writers</Text>
                                        <Text>{
                                            writers ? (writers.map((data, index) => (

                                                <span style={styles.letter}>{data}</span>
                                        ...
                                        {/* <hr style={styles.hrr} /> */}
                                        <Text style={styles.marg}>Cast</Text>
                                        <Text> {
                                            cast.map((data, index) => (
                                        ...
                                        {/* <hr style={styles.hrr} /> */}
                                        <Text style={styles.marg}>Plot</Text>
